...
>
    ...
    document.getElementById('saveMonth').addEventListener('click', () => {
        ...
        fetch(`/laravel/dayaa_sat_Vision/admin/customers/update/${currentCustomer.id}`, {
            method: 'POST',
            headers: {
                ...
                if (data.success) {
                    ...
                    setTimeout(() => {
                        ...
                        if (window.appRole === "staff") {
                            window.location.href = `/laravel/dayaa_sat_Vision/admin/staff/search?search=${search}`;
                        } else {
                            window.location.href =
                                `/laravel/dayaa_sat_Vision/admin/dashboard?place=${place}&search=${search}`;
                        }
                    }, 1000);
...
function handleSaveChanges() {
    const url = `/laravel/dayaa_sat_Vision/admin/customers/update/${currentCustomer.id}`;
    const token = document.querySelector('meta[name="csrf-token"]').getAttribute('content');
