document.addEventListener('DOMContentLoaded', () => {
    ...
    function loadCustomers() {
        ...
        */
        ...
        fetch(`${listUrl}?place=${encodeURIComponent(placeFilter)}&year=${year}`)
            .then(res => res.json())
            .then(response => {
    ...
    function calculateLatestTotal(customer) {
        const yearSelector = document.getElementById("yearSelector");
        let selectedYear = yearSelector ? yearSelector.value : new Date().getFullYear();

        if (!customer.progress_json || !customer.progress_json[selectedYear]) return 0;

        const paymentDates = customer.progress_json[selectedYear].paymentDates;
    ...
    function calculateYearTotal(customer) {
        const yearSelector = document.getElementById("yearSelector");
        let selectedYear = yearSelector ? yearSelector.value : new Date().getFullYear();
        ...
        let total = 0;
        let paymentDates = customer.progress_json[selectedYear].paymentDates;
        ...
        paymentDates.forEach(month => {
            if (Array.isArray(month) && month.length > 0) {
                ...
                total += parseFloat(month.amount) || 0;
            }
        });
        ...
        let filtered = allCustomers.filter(c => {
            ...
            if (from || to) {
                ...
                if (paymentStatus === "") {
                    ...
                    const latestPay = getLatestPaymentDate(c.progress_json?.[year]?.paymentDates || []);
...
<tr class="table-row">
   ...
   <td class="table-cell">₹${calculateYearTotal(customer).toLocaleString()}</td>
    ...
    function renderOverallStats(customers) {
        ...
        if (customers.length === 0) {
            ...
            <div class="stats-row">
                ...
                <div class="stat-card">
                    ...
                    <div class="stat-value collected">₹0</div>
                </div>
            </div>
        `;
            return;
        ...
        customers.forEach(c => {
            if (!c.progress_json || !c.progress_json[selectedYear]) return;

            const yearPayments = c.progress_json[selectedYear].paymentDates;

            if (!Array.isArray(yearPayments)) return;

            yearPayments.forEach(month => {
                if (Array.isArray(month) && month.length > 0) {
                    const latest = month[month.length - 1];
                    totalCollected += parseFloat(latest.amount) || 0;
                } else if (month && typeof month === "object") {
                    totalCollected += parseFloat(month.amount) || 0;
                }
            });
        });
        ...
        filteredCustomers.forEach(c => {
            ...
            yearPayments.forEach(month => {
                if (Array.isArray(month) && month.length > 0) {
                    ...
                    filteredCollected += parseFloat(month.amount) || 0;
                }
            });
        });
   ...
   ============================= */
        document.querySelectorAll('.edit').forEach(btn => {
            btn.addEventListener('click', e => {
                ...
                window.location.href =
                    `/laravel/dayaa_sat_Vision/admin/customers/edit/${id}?place=${place}`;
            });
        });
        document.querySelectorAll('.rupee').forEach(btn => {
            btn.addEventListener('click', e => {
                ...
                window.location.href =
                    `/laravel/dayaa_sat_Vision/admin/customers/edit/${id}?mode=payment&place=${place}`;
            });
        });
        ...
        document.querySelectorAll('.delete').forEach(btn => {
            btn.addEventListener('click', e => {
                ...
                Swal.fire({
                    ...
                    fetch(`/laravel/dayaa_sat_Vision/admin/customers/destroy/${id}`, {
                        method: 'DELETE',
    ...
    document.getElementById('downloadBtn').addEventListener('click', () => {
        ...
        const exportData = allCustomers.map((c, index) => ({
            ...
            'Total Amount': calculateLatestTotal(c),
    ...
    if (downloadTodayBtn) {
        ...
        downloadTodayBtn.addEventListener('click', () => {
            ...
            const filtered = allCustomers.filter(c => {
                ...
                const year = document.getElementById("yearSelector").value;
                const yearPayments = c.progress_json?.[year]?.paymentDates || [];

                yearPayments.forEach(monthPayments => {
                    if (Array.isArray(monthPayments)) {
